...
class SLURMTask(user: HtrcUser, inputs: JobInputs, id: JobId) extends Actor {
  ...
  val dependenciesReady = inputs.dependencies map { algDepName =>
    (registry ? WriteFile(algDepName, workingDir, inputs)).mapTo[WriteStatus]
  } toList
  ...
  def launchJob(resourceAlloc: ResourceAlloc): String = {
    ...
    val cmdF = "ssh -t -t -q %s sbatch %s -J %s -D %s %s --time=%s " +
    "--mem=%s --output=%s --error=%s %s/%s/%s"
    val cmd = cmdF.format(target,HtrcConfig.getSbatchOptions, constructJobName, targetWorkingDir + "/" + id,
